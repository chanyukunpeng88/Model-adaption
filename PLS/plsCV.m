function RMSECV = plsCV(X, Y, h, premethod, valmethod, group, iter)
...
if ~isempty(iter)
    for i = 1 : iter
        ...
        indm(indt) = [];              % indices of the train set
        
        currXTrn= X(indm,:);  currYTrn= Y(indm,:);
        currXTst = X(indt,:);  currYTst = Y(indt,:);
        Result = pls2reg(currXTrn, currYTrn,currXTst, currYTst, h, premethod);
        tstY = Result.PTst;
        for j=1:h
            Temp=(currYTst - tstY{1,j}).^2;
            sumsqerr(i,j) = sum(Temp(:));
        end
        RMSECV = sqrt( sum(sumsqerr) / (group*iter*p) );
    end
